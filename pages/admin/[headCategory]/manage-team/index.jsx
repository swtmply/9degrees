...
export default function index() {
  ...
  const getUsers = async () => await axios.get('/api/user/').then((res) => res.data)
          ...
          <div className="grid grid-cols-5">
            <div className='w-full col-start-5'>
              <button 
                ...
                onClick={() => router.push(`/admin/${session?.user.categories}/manage-team/create`)}
                >
                <div>add a user +</div>
              </button>
            </div>
            ...
            <div className="rounded-md px-3">
              {isLoading ? <div className="mt-10"> <Loading /> </div> : (
                <table className="table table-fixed w-full border-separate space-y-6 text-sm">
                  ...
                  <tbody>
                    {data?.users.map((user) => (
                      <tr className="bg-white hover:bg-[#e6e6e6]" key={user._id} >
                        <td className="px-4 pt-2 pb-4">
                          <div className="flex items-center space-x-2">
                            <button onClick={() => router.push(`/admin/${session?.user.categories}/manage-team/edit/${user._id}`)}>
                        ...
                        <td className="px-4 pt-2 pb-4">
                          <Link href={`/admin/${session?.user.categories}/manage-team/edit/${user._id}`}>
                            <a>{user.name}</a>
                          </Link>
                        </td>
                        ...
                        <td className="px-4 pt-2 pb-4 text-padeepBlue">
                          {user.categories.length > 1
                            ? user.categories.map((cat) => <div className="inline">{cat}{', '}</div>)
                            : user.categories
                          }
                        </td>
